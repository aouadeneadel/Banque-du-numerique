...
export function DeviceManagement() {
  ...
  // Charger la liste depuis la base au montage
  useEffect(() => {
    fetch('http://localhost/BanqueDuNumerique/backend/get_devices.php')
      .then(res => res.json())
  ...
  const handleModify = () => {
    if (
      ...
      fetch('http://localhost/BanqueDuNumerique/backend/update_device.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = () => {
    if (selectedId) {
      fetch('http://localhost/BanqueDuNumerique/backend/delete_device.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const filteredDevices = devices.filter(device => {
    ...
    const matchesSearch =
      device.modele.toLowerCase().includes(searchTerm.toLowerCase()) ||
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${
                      device.etat === 'Neuf'
                        ...
                        : device.etat === 'Occasion'
                        ? 'bg-blue-100 text-blue-800'
                        : device.etat === 'Réparé'
                        ? 'bg-yellow-100 text-yellow-800'
                        : device.etat === 'En panne'
