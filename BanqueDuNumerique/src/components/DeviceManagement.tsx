...
import { mockDevices, mockInterlocuteurs, generateInventoryNumber, generateSerialNumber, type Device, type Interlocuteur } from '../data/mockDatabase';

const etats = ['Neuf', 'Occasion', 'Réparé', 'En panne', 'Livré'];

export function DeviceManagement() {
  const [interlocuteurs] = useState<Interlocuteur[]>(mockInterlocuteurs);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.modele || !formData.marque || !formData.annee || !formData.etat) return;

    // Auto-génération des numéros si non fournis
    const numeroSerie = formData.numeroSerie || generateSerialNumber(formData.marque, parseInt(formData.annee));
    const numeroInventaire = formData.numeroInventaire || generateInventoryNumber(devices);
    ...
    const newDevice: Device = {
      ...
      interlocuteurId: formData.interlocuteurId !== 'none' ? parseInt(formData.interlocuteurId) : undefined
    };
  ...
  const resetForm = () => {
    setFormData({
      ...
      modele: '', marque: '', annee: '', etat: '', numeroSerie: '', numeroInventaire: '', interlocuteurId: 'none'
    });
  };
  ...
  const handleRowClick = (device: Device) => {
    ...
    setFormData({
      ...
      interlocuteurId: device.interlocuteurId?.toString() || 'none'
    });
  };
  ...
  const getInterlocuteurName = (interlocuteurId?: number) => {
    if (!interlocuteurId) return '-';
    const interlocuteur = interlocuteurs.find(i => i.id === interlocuteurId);
    return interlocuteur ? interlocuteur.nomStructure : '-';
  };
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-lg border shadow-sm">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="interlocuteur">Structure (optionnel)</Label>
                  <Select value={formData.interlocuteurId} onValueChange={(value) => handleInputChange('interlocuteurId', value)}>
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      {interlocuteurs.map((interlocuteur) => (
                        <SelectItem key={interlocuteur.id} value={interlocuteur.id.toString()}>
                          {interlocuteur.nomStructure}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-primary hover:bg-primary">
              ...
              <TableHead className="text-primary-foreground">Structure</TableHead>
          ...
          <TableBody>
            {filteredDevices.map((device, index) => (
              ...
              >
                ...
                <TableCell>{getInterlocuteurName(device.interlocuteurId)}</TableCell>
