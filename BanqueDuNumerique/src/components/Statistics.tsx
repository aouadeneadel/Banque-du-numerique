...
import { getStatistics, mockInterlocuteurs, mockDevices } from '../data/mockDatabase';
...
export function Statistics() {
  ...
  // Statistiques sur les dons par interlocuteur
  const donsParInterlocuteur = mockInterlocuteurs.map(inter => {
    const totalDonnes = mockDevices.filter(d => d.interlocuteurId === inter.id).length;
    return {
      nomStructure: inter.nomStructure,
      totalDonnes
    };
  });

  const totalDons = mockDevices.filter(d => d.interlocuteurId).length;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats Dons & Interlocuteurs */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
        <Card>
          <CardHeader>
            <CardTitle>Total Interlocuteurs</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalInterlocuteurs}</div>
            <p className="text-xs text-muted-foreground">Structures partenaires</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Total Appareils Donnés</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalDons}</div>
            <p className="text-xs text-muted-foreground">Appareils attribués</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Dons par Structure</CardTitle>
          </CardHeader>
          <CardContent>
            {donsParInterlocuteur.map(item => (
              <div key={item.nomStructure} className="flex justify-between">
                <span>{item.nomStructure}</span>
                <span className="font-bold">{item.totalDonnes}</span>
              </div>
            ))}
          </CardContent>
        </Card>
